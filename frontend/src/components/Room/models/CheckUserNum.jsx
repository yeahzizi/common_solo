...
function CheckUserNum({ thisRoom, userNum }) {
  ...
  async function checkNum() {
    const response = await axios.get(
      `https://i8b206.p.ssafy.io:9000/api/room/${thisRoom}`
    );
    const a = response;
    if (response.data.userJoinLists.length > userNum) {
      const delNum = await axios.delete(
        `https://i8b206.p.ssafy.io:9000/api/room/${thisRoom}/${response.data.userJoinLists.length}`
      );
      const res = delNum;
    } else if (response.data.userJoinLists.length < userNum) {
      const addNum = await axios.get(
        `https://i8b206.p.ssafy.io:9000/api/room/${thisRoom}/${userNum}`
      );
      const res = addNum;
    }
  }
